...
# add one row of images that concatenate the original and reconstructed images
def plot_ae_outputs(model, classes=10, samples_per_class=5):
    ...
    for i in range(classes):
        for j in range(samples_per_class):
            ...
            half_img, _ = split_image(img)
            model.eval()
            with torch.no_grad():
                rec_img = model(half_img)
            ...
            concatenated_img = torch.cat((half_img, rec_img), dim=3)
            plt.imshow(concatenated_img.cpu().squeeze().numpy(), cmap="gist_gray")
            ax.get_xaxis().set_visible(False)
            ax.get_yaxis().set_visible(False)
            if i == classes // 2:
...
train_dataset = torchvision.datasets.FashionMNIST(
    root="./fmnist", train=True, transform=torchvision.transforms.ToTensor(), download=True
)
test_dataset = torchvision.datasets.FashionMNIST(
    root="./fmnist", train=False, transform=torchvision.transforms.ToTensor(), download=True
)
...
class AutoEncoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim):
        ...
        self.encoder_cnn = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=(4, 2), stride=2, padding=1),
            nn.BatchNorm2d(32),
            nn.LeakyReLU(True),
            nn.Conv2d(32, 64, kernel_size=(4, 2), stride=2, padding=0),
            nn.BatchNorm2d(64),
            nn.LeakyReLU(True),
            nn.Conv2d(64, 128, kernel_size=(4, 2), stride=2, padding=1),
            nn.LeakyReLU(True),
        )
        ...
        self.decoder_linear = nn.Sequential(
            ...
            nn.LeakyReLU(True),
            nn.Linear(hidden_dim, 128 * 3 * 3),
        )
        self.unflatten = nn.Unflatten(dim=1, unflattened_size=(128, 3, 3))
        self.decoder_cnn = nn.Sequential(
            nn.ConvTranspose2d(128, 64, kernel_size=(4, 2), stride=2, padding=1),
            nn.BatchNorm2d(64),
            nn.LeakyReLU(True),
            nn.ConvTranspose2d(64, 32, kernel_size=(4, 2), stride=2),
            nn.BatchNorm2d(32),
            nn.LeakyReLU(True),
            nn.ConvTranspose2d(32, 1, kernel_size=(4, 2), stride=2, padding=1),
        )
